...
int main() {
  ...
  CHECK_CUDA(cudaMemcpy(dA, dA_host, num_rows*num_cols, cudaMemcpyHostToDevice));
  ...
  CHECK_CUSPARSE(cusparseLtStructuredDescriptorInit(&handle, &matA, num_rows, num_cols, num_rows, 16, CUDA_R_16F, CUSPARSE_ORDER_COL, CUSPARSELT_SPARSITY_50_PERCENT));
  ...
  CHECK_CUDA(cudaMemcpy(dA_host, dA, num_rows*num_cols, cudaMemcpyDeviceToHost));
  for (int i=0; i < 4*4; i++)
    printf("%f ", __half2float(dA_host[i]));
  printf("\n");
